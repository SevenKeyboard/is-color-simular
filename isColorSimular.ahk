...
{
    static _ := VersionManager_isColorSimular._init()
    _init()    {
        global
...
isColorSimular(argb1, argb2, alphaVariation:=0, rgbVariation*)    {
    c:=Object(1,"", 2,"")
    va:=Object("a",format("{:d}",alphaVariation), "r","", "g","", "b","")
    ;--------------------------------
    Loop 2    {
        i:=A_Index
        c[i]:=Object("a","", "r","", "g","", "b","")
        ...
        c[i]["b"]:=format("{:d}","0x" subStr(_hex,-1))
        c[i]["g"]:=format("{:d}","0x" subStr(_hex,-3,2))
        c[i]["r"]:=format("{:d}","0x" subStr(_hex,-5,2))
        _ahex:=(regExMatch(_hex,"DO)([a-fA-F0-9]{1,2})[a-fA-F0-9]{6}$",_)?_[1]:"")
        if (_ahex=="")
            c[i]["a"]:=format("{:d}",0xFF)
        else
    ...
    switch (rgbVariation.maxIndex())
    {
        default:        return false
        case "":        va["r"]:= va["g"]:= va["b"]:= 0
